...
var addPlayer = function (playerInfo) {
	...
	player1.socketId = playerInfo.playerId;
	return player1;
}

var addOtherPlayers = function (otherPlayers, playerInfo) {
	...
	Object.keys(otherPlayers).forEach(function(id){
		otherPlayers[id].socketId = playerInfo.playerId;
	})
}
...
var turnLeft = function (cycle, angle) {
	return function () {
		cycle.rotateY(angle);
		cycle.turned = true;
...
var turnRight = function (cycle, angle) {
	return function () {
		cycle.rotateY(-angle);
		cycle.turned = true;
		cycle.windingOrder -= 1;
...
var activateAI = function (cycle) {
	if ((elapsedTime - cycle.lastTurnTime) > 0.1) {
		...
		if (cycle.windingOrder < -3) {
			...
			cycle.turnQueue.push(turnLeft(cycle, halfPi));
			cycle.turnQueue.push(turnLeft(cycle, halfPi));
			...
			cycle.turnQueue.push(turnRight(cycle, halfPi));
			cycle.turnQueue.push(turnRight(cycle, halfPi));
