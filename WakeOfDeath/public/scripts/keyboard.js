...
var handleKeyDown = function(e) {
	...
	var v = Math.PI/30;
	if(!e.repeat){
		myTime = setInterval(function(){ t+=0.1;}, 100);
	}
	if (e.repeat) {
		var angle = 0.5 * a * t * t + v * t;
		if(angle > Math.PI/2){
			angle = Math.PI/2;
		}
		if(e.keyCode === 65){
			player1.turnQueue.push(turnLeft(player1, angle));
		}else if(e.keyCode === 68){
			player1.turnQueue.push(turnRight(player1, angle));
		}
	...
	switch ( e.keyCode ) {
		case 65: // left	
					player1.turnQueue.push(turnLeft(player1, Math.PI/30));
					break;

		case 68: // right
					player1.turnQueue.push(turnRight(player1, Math.PI/30));
					break;
